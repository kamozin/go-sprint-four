...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedSquare := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	calories := ((walkingCaloriesWeightMultiplier*weight + (speedSquare/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
